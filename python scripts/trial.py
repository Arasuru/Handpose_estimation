...
def network_communication(socket, recording_state, filename_queue):
    try:
        while True:
            data = socket.recv(1024)
            if data == b'START' and not recording_state['recording']:
                # Start recording
                print(f"start signal received at {datetime.datetime.now().time()}")
                recording_state['recording'] = True
                filename = f"{uuid.uuid4()}.mp4"
                filename_queue.append(filename)  # Send filename to the main thread
                print(f"Recording started: {filename}")
            elif data == b'STOP' and recording_state['recording']:
                # Stop recording
...
# Thread-safe recording state and filename queue
recording_state = {'recording': False}
filename_queue = []

# Start network communication thread
network_thread = threading.Thread(target=network_communication, args=(s, recording_state, filename_queue))
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks: #gives the coordinates of hand if a hand is detected
            for num, hand in enumerate(results.multi_hand_landmarks):
                ...
                if recording_state['recording'] and landmarks_3d:
        ...
        key = cv2.waitKey(1)  # Wait 1ms for a key press.  Important for timing.
       # cv2.pollKey() removed, it is non-blocking

        # Start recording based on the signal received from the thread
        if not out and filename_queue:
            filename = filename_queue.pop(0)
            out = cv2.VideoWriter(os.path.join("../mediapipe videos", filename), fourcc, fps, (frame_width, frame_height))
           

        if recording_state['recording'] and out is not None:
            out.write(image) #use image instead of blank_frame to save actual video
        elif not recording_state['recording'] and out is not None:
           # Stop recording
